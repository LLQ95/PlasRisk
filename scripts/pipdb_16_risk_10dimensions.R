...
# 10-dimension risk assessment: adds S_BMG (biocides/metals) to the 9-dim model.
...
W9 <- W9 / sum(W9)
# 10-dim weights: add S_BMG=0.10, redistribute proportionally
W10 <- c(W9 * 0.9, S_BMG=0.10)
...
cat(sprintf("  S_BMG: mean=%.3f, %d PSCs with S_BMG>0 (%.1f%%)\n", mean(d$S_BMG,na.rm=TRUE), sum(d$S_BMG>0,na.rm=TRUE), 100*mean(d$S_BMG>0,na.rm=TRUE)))
...
rep10 <- dt[, .(S10=mean(S10,na.rm=TRUE), n_PSC=.N,
  mean_S_ARG=mean(S_ARG,na.rm=TRUE), mean_S_VF=mean(S_VF,na.rm=TRUE), mean_S_BMG=mean(S_BMG,na.rm=TRUE),
...
# S_BMG analysis
cat("\n=== S_BMG analysis ===\n")
bm_by_rep <- dt[, .(n_PSC=.N, mean_S_BMG=mean(S_BMG,na.rm=TRUE), pct_BM=100*mean(S_BMG>0,na.rm=TRUE),
  pct_mer=100*mean(has_mer,na.rm=TRUE), pct_qac=100*mean(has_qac,na.rm=TRUE), pct_ars=100*mean(has_ars,na.rm=TRUE)),
  by=replicon_primary][n_PSC>=50][order(-mean_S_BMG)]
cat("Top 10 replicons by S_BMG:\n"); print(head(bm_by_rep, 10))

# S_BMG temporal trend
...
bm_temporal <- d[!is.na(era), .(n_PSC=.N, mean_S_BMG=mean(S_BMG,na.rm=TRUE), pct_BM=100*mean(S_BMG>0,na.rm=TRUE)), by=era]
p25a <- ggplot(bm_temporal, aes(x=era, y=mean_S_BMG, group=1)) +
  geom_line(linewidth=0.8, colour="#756bb1") + geom_point(size=3, colour="#756bb1") +
  labs(x="Isolation era", y="Mean S_BMG", title="Temporal trend of biocide/metal resistance") + theme_pub
p25b <- ggplot(bm_temporal, aes(x=era, y=pct_BM, group=1)) +
  geom_col(fill="#bcbddc", colour="#756bb1", linewidth=0.3) +
  labs(x="Isolation era", y="% PSCs with S_BMG > 0", title="Prevalence of BM resistance genes") + theme_pub
p25c <- ggplot(d[S_BMG>0], aes(x=S_BMG)) +
  ...
  labs(x="S_BMG", y="Count", title="S_BMG distribution by top replicons") + theme_pub
p25d <- ggplot(bm_by_rep[1:20], aes(x=reorder(replicon_primary, mean_S_BMG), y=mean_S_BMG)) +
  geom_col(fill="#756bb1", colour="white") + coord_flip() +
  labs(x=NULL, y="Mean S_BMG", title="Top 20 replicons by S_BMG") + theme_pub
...
rf1 <- randomForest(highrisk ~ S_ARG+S_VF+S_MOB+S_HOST+S_REP+S_SIZE+S_BMG+S_GEO+S_HAB+S_GROW, data=ml_sample, ntree=300, importance=TRUE)
rf2 <- randomForest(mdr_vf ~ S_ARG+S_VF+S_MOB+S_HOST+S_REP+S_SIZE+S_BMG+S_GEO+S_HAB+S_GROW, data=ml_sample, ntree=300, importance=TRUE)
rf3 <- randomForest(conj ~ S_ARG+S_VF+S_MOB+S_HOST+S_REP+S_SIZE+S_BMG+S_GEO+S_HAB+S_GROW, data=ml_sample, ntree=300, importance=TRUE)
rf4 <- randomForest(bm ~ S_ARG+S_VF+S_MOB+S_HOST+S_REP+S_SIZE+S_BMG+S_GEO+S_HAB+S_GROW, data=ml_sample, ntree=300, importance=TRUE)
...
for (bw in bm_weights) {
  ...
  sens_results[[length(sens_results)+1]] <- data.table(S_BMG_weight=bw, spearman_rho=rho, top10_overlap=top10)
}
sens_dt <- rbindlist(sens_results)
fwrite(sens_dt, file.path(tab_dir,"tab_SBM_weight_sensitivity.csv"))
p28 <- ggplot(sens_dt, aes(x=S_BMG_weight, y=spearman_rho)) +
  ...
  labs(x="S_BMG weight", y="Spearman rho vs 10-dim ranking", title="S_BMG weight sensitivity") + theme_pub
